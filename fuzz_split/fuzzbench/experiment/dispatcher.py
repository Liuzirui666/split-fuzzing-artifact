...
LOOP_WAIT_SECONDS = 30  # Reduced from 5*60 for faster experiment turnover
...
def dispatcher_main():
    ...
    is_complete = False
    scheduler_done_time = None
    MAX_WAIT_AFTER_SCHEDULER = 120  # seconds to wait for measurer after scheduler exits

    while True:
        time.sleep(LOOP_WAIT_SECONDS)

        scheduler_alive = scheduler_loop_thread.is_alive()

        if not scheduler_alive:
            if scheduler_done_time is None:
                scheduler_done_time = time.time()
                logs.info('Scheduler finished. Waiting for measurer...')

            # Check measurer with multiple methods
            measurer_done = (not measurer_main_process.is_alive() or
                             measurer_main_process.exitcode is not None)

            # Hard timeout: if scheduler done for too long, force complete
            elapsed = time.time() - scheduler_done_time
            if measurer_done or elapsed > MAX_WAIT_AFTER_SCHEDULER:
                if not measurer_done:
                    logs.warning(
                        'Measurer still alive after %ds, forcing completion.',
                        int(elapsed))
                is_complete = True
    ...
    scheduler_loop_thread.join(timeout=10)
    measurer_main_process.terminate()
    measurer_main_process.join(timeout=10)

    _record_experiment_time_ended(experiment.experiment_name)
    logs.info('Experiment ended.')

    # Force exit to clean up leaked multiprocessing Pool workers
    import os
    os._exit(0)
