#!/bin/bash
# Run 12h online+nosplit experiments on BOTH benchmarks sequentially:
# poppler (T=12h) → openssl (T=12h). Full pipeline + figures after each.
#
# Same structure as run_6h_chain.sh: K=3 stages (max 8 leaves per split),
# 4 fuzzers × 5 trials × {online, nosplit}. To get 10 trials total, run
# this script twice; the two invocations produce separate timestamped
# experiment directories.
set -uo pipefail

ROOT=/path/to/magma_split
TOTAL_HOURS=${TOTAL_HOURS:-12.0}
TRIALS=${TRIALS:-5}
BRANCHING=2
MAX_SPLITS=3
LEAVES=$((BRANCHING ** MAX_SPLITS))

# Sparsity params — same shape as 6h chain, tight so splits fire early.
WINDOW_HOURS=0.05
PERSIST_HOURS=0.05
MIN_GAP_HOURS=0.167
...
run_one_benchmark() {
    ...
    local EXP_DIR="$ROOT/experiments/main_12h_online_${TARGET}_$(date +%Y%m%d_%H%M%S)"
